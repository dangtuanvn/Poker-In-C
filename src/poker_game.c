...
int main(int argc, char *argv[]) {
    ...
    //create windows
    my_wins[0] = newwin(MENU_HEIGHT, MENU_WIDTH, (getmaxy(stdscr) - MENU_HEIGHT) / 2 + 7,
    ...
    my_wins[2] = newwin(INPUT_WIN_HEIGHT, INPUT_WIN_WIDTH, getmaxy(stdscr) - 9,
                        getmaxx(stdscr) / 2 - (CARD_WIDTH * 5 + 6) / 2 - 35);
...
//Game Flow-------------------
                while (game_round->remaining_players != 1 && players_list[0]->status > 0) {
                    ...
                    attron(COLOR_PAIR(2));
                    mvprintw(getmaxy(stdscr) / 2 + START_Y_PHASE + 1,
                             (int) (getmaxx(stdscr) - strlen("BETTING-PHASE-1")) / 2, "BETTING-PHASE-1");
                                    ...
                                    switch (in) {
                                        case KEY_MOUSE: // mouse event
                                            if (getmouse(&event) == OK && (event.bstate & BUTTON1_CLICKED
                                                ...
                                                if (event.y == 2 && event.x >= getmaxx(stdscr) - 20 &&
                                                    event.x < getmaxx(stdscr) - 11) {
                                        ...
                                        case 'v':
                                            if (players_list[position]->bet_amount == game_round->call_amount) {
                                                action_bet(game_round, players_list[position],
                                                           players_list[position]->chips);
                                            }
                                            else {
                                                action_raise(game_round, players_list[position],
                                                             players_list[position]->chips);
                                            }
                                            check_input0 = 0;
                                            break;
                    ...
                    attron(COLOR_PAIR(2));
                    mvprintw(getmaxy(stdscr) / 2 + START_Y_PHASE + 1,
                             (int) (getmaxx(stdscr) - strlen("CHANGE-CARD-PHASE")) / 2, "CHANGE-CARD-PHASE");
                    attroff(COLOR_PAIR(2));

                    attron(COLOR_PAIR(3));
                    attron(A_BOLD);
                    mvprintw(getmaxy(stdscr) / 2 + 5, (int) (getmaxx(stdscr) - strlen(" CHANGE(ENTER) ")) / 2,
                             " CHANGE(ENTER) ");
                    attroff(A_BOLD);
                    ...
                    mvprintw(getmaxy(stdscr) / 2 + 5, (int) (getmaxx(stdscr) - strlen("                    ")) / 2,
                             "                    ");
                    mvprintw(getmaxy(stdscr) / 2 + START_Y_PHASE + 1,
                             (int) (getmaxx(stdscr) - strlen("CHANGE-CARD-PHASE")) / 2, "                    ");
                    attron(COLOR_PAIR(2));
                    mvprintw(getmaxy(stdscr) / 2 + START_Y_PHASE + 1,
                             (int) (getmaxx(stdscr) - strlen("BETTING-PHASE-2")) / 2, "BETTING-PHASE-2");
                    attroff(COLOR_PAIR(2));
                    if (!check_fold) {
                        position = 0;
                    ...
                    attron(A_BOLD);
                    mvprintw(getmaxy(stdscr) / 2 + START_Y_PHASE + 1,
                             (int) (getmaxx(stdscr) - strlen(" NEXT-ROUND(N) ")) / 2, " NEXT-ROUND(N) ");
                    attroff(A_BOLD);
                    attroff(COLOR_PAIR(3));
                    check_input3 = 1;
                        ...
                        switch (in) {
                            case KEY_MOUSE: // mouse event
                                if (getmouse(&event) == OK && (event.bstate & BUTTON1_CLICKED
                                    ...
                                    else if (event.y == 2 && event.x >= getmaxx(stdscr) - 20 &&
                                             event.x < getmaxx(stdscr) - 11) {
                                        current_stage.num = MAIN_MENU;
                                        current_win = my_wins[0];
                                        clear();
                ...
                if (players_list[0]->status == BUSTED) { // if player is busted out, print game over
                    mvprintw(getmaxy(stdscr) / 2 + START_Y_PHASE, (int) (getmaxx(stdscr) - strlen(" GAME-OVER! ")) / 2,
                             " GAME-OVER! ");
                    mvprintw(getmaxy(stdscr) / 2 + START_Y_PHASE + 1,
                             (int) (getmaxx(stdscr) - strlen(" RETURN-TO-MENU(ENTER) ")) / 2,
                             " RETURN-TO-MENU(ENTER) ");

                }
                else {
                    mvprintw(getmaxy(stdscr) / 2 + START_Y_PHASE,
                             (int) (getmaxx(stdscr) - strlen(" CONGRATULATIONS ")) / 2, " CONGRATULATIONS ");
                    mvprintw(getmaxy(stdscr) / 2 + START_Y_PHASE + 1,
                             (int) (getmaxx(stdscr) - strlen(" RETURN-TO-MENU(ENTER) ")) / 2,
                             " RETURN-TO-MENU(ENTER) ");
                }
                ...
                case KEY_MOUSE: // mouse event for menu
                    if (getmouse(&event) == OK && (event.bstate & BUTTON1_CLICKED
                        ...
                        if (pointed_item(current_win, event.x, event.y, current_stage) >= 0) {
                            ...
                            display_menu(current_win, &current_stage);
                            if (current_stage.num == NEW_GAME &&
                                (current_stage.selection == 0 || current_stage.selection == 2)) {
                                current_win = my_wins[1];
                            }
                            else if (current_stage.num == MODE ||
                                     (current_stage.num == SINGLE_PLAYER && current_stage.selection == 1)) {
                                current_win = my_wins[0];
                            }
                            change_stage(current_win, &current_stage);
                        }
...
int mouse_in_game(int posY, int posX, int phase, int turn) { // mouse listener for in game
    ...
    switch (phase) {
        case 1:
            ...
            break;
        case 2:
            if (posY == getmaxy(stdscr) / 2 + 5 && posX >= (getmaxx(stdscr) - 15) / 2 &&
                posX < (getmaxx(stdscr) - 15) / 2 + 15) {
                process_change_card(deck, players_list);
                display_deck(player_seat[0], *players_list[0]);
                check = 0;
            }
